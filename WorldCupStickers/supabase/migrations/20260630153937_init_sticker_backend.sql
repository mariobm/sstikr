...
revoke all on function public.is_direct_friend(uuid, uuid) from public;
revoke all on function public.is_friend_of_friend(uuid, uuid) from public;
...
grant select on public.user_stickers to anon, authenticated;
grant insert, update on public.user_stickers to authenticated;
grant select, insert on public.collection_mutations to authenticated;
